        ...
        {items.map((item) => {
          ...
          return (
            ...
            >
              ...
              {item.badge ? <span className={['cinema-nav-badge', item.badge === '5' ? 'is-pink' : 'is-amber'].join(' ')}>{item.badge}</span> : null}
            </button>
          )
        })}
      </div>
...
function CinemaIcon({ name }: { name: string }) {
  const icon = {
    ...
    settings: <><path d="m.12 3 1.5 2.7 3.1.4.7 3 .7.5-.7.5-.7 3-3.1.4L12 21l-1.5-2.7-3.1-.4-.7-3-.7-.5.7-.5.7-3 3.1-.4L12 3Z" /><path d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z" /></>,
